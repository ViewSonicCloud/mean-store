...
exports.getCartDetail = function(where, cb) {
    Cart.aggregate()
        ...
        .lookup({
            from: "licenses",
            foreignField: "_id",
            localField: "selected_pricing.license",
            as: "license"
        })
        .group({
            "_id": null,
            "cart": {
                "$push": {
                    ...
                    "selected_pricing": {
                        "name": {
                            $cond: {
                                if: "$selected_pricing.name",
                                then: "$selected_pricing.name",
                                else: {
                                    $cond: {
                                        if: {
                        ...
                        license: {
                            $cond: {
                                if: "$selected_pricing.license",
                                then: "$selected_pricing.license",
                                else: null
                            }
                        },
                        original: "$selected_pricing.original",
                        after_discount: "$selected_pricing.after_discount",
                        savings: "$selected_pricing.savings",
                        commission: "$selected_pricing.commission",
                        service_tax: {
                            $cond: {
                                if: "$selected_pricing.service_tax",
                                then: "$selected_pricing.service_tax",
                                else: null
                            }
                        },
                        quantity: {
                            $cond: {
                                if: "$selected_pricing.quantity",
                                then: "$selected_pricing.quantity",
                                else: "$product_id.quantity"
                            }
                        }
                    },
                    "seller": "$seller",
